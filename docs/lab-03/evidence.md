...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Issue 10** | `feature/10-lab3-documentation` | Documentation & Engineering Contract | `[PR #... Placeholder]` | `[Pending Review]` | `[Pending Merge]` |
| **Issue 11** | `feature/11-database-migration` | Database Migration & Idempotent Seed | `[PR #... Placeholder]` | `[Pending]` | `[Pending]` |
| **Issue 12** | `feature/12-authentication-authorization` | Auth Foundation, Session & Discussion API | `[PR #... Placeholder]` | `[Pending]` | `[Pending]` |
...
* **Test Log Output Placeholder:**
```text
[PLACEHOLDER: Paste Vitest output for backend test suites:
 - auth.api.test.ts (Login, Cookie, Logout, Deactivated user, Password reset)
 - authorization.api.test.ts (RBAC, Requester ownership, 404 leakage guard)
 - comments-notes.api.test.ts (Public discussion, Internal notes isolation, Confirm-resolved)
 - staff-queue.api.test.ts (Listing, Search, Filter, Pagination)
 - staff-ticket-detail.api.test.ts (Claim, Reassign, IT Priority, State transitions, Mandatory reasons)
 - users-admin.api.test.ts (User CRUD, Self-deactivation guard, Last-admin protection)]
```
